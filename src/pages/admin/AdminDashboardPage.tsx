...
export function AdminDashboardPage() {
  ...
  const lastSite = site?.updatedAt
    ? "Conteúdo guardado no Firestore"
    : "Ainda sem rascunho guardado — use Conteúdo do site";

  return (
    <div>
      <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">Painel</h1>
      <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
        Números em tempo real: coleções <code className="text-xs">users</code> e <code className="text-xs">posts</code> (o site público lê as
      ...
      {dataError && (
        <p className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-200">
          Aviso Firestore: {dataError} — confirme regras e deploy.
        </p>
      )}
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        ...
        <Card padding="lg">
          ...
          <p className="mt-1 text-xs text-zinc-500">{publishedPosts} publicadas no site</p>
      ...
      <Card padding="lg" className="mt-8">
        <h2 className="text-lg font-semibold text-zinc-900 dark:text-white">Atalhos</h2>
